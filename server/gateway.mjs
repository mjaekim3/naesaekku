...
export async function createGateway({ studio, token, port = 0 }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const given = Buffer.from(req.headers.authorization || ""),
      wanted = Buffer.from(`Bearer ${token}`);
    if (given.length !== wanted.length || !timingSafeEqual(given, wanted))
      return send(401, { error: "인증이 필요합니다." });
    const path = new URL(req.url, "http://127.0.0.1").pathname;
    try {
      if (req.method === "GET" && path === "/api/state")
        return send(200, await studio.state());
      if (req.method === "GET" && path.startsWith("/api/asset/")) {
        const a = await studio.asset(path.slice(11));
        res.writeHead(200, { "Content-Type": a.mime });
        return res.end(a.buffer);
      }
      if (req.method === "GET" && path.startsWith("/api/job/")) {
        const j = studio.job(path.slice(9));
        return send(j ? 200 : 404, j || { error: "작업을 찾을 수 없습니다." });
      }
      if (
        req.method !== "POST" ||
        ![
      ...
      try {
        ...
        return send(400, { error: "요청 형식을 확인해주세요." });
      }
      if (path === "/api/import")
        return send(
          200,
      ...
      if (path === "/api/cancel") {
        studio.cancel(body.id);
        return send(200, { ok: true });
      }
      const out = await studio.export(body.id, body.format, body.blinkId);
