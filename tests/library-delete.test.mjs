...
test("permanent deletion requires trash and confirmation, removes only its files and persists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "permanent-"));
  const id = randomUUID(),
    other = randomUUID();
  try {
    const s = await Studio.open({ dir });
    ...
    await mkdir(join(dir, "pets", id), { recursive: true });
    await writeFile(join(dir, "pets", id, "0.png"), "frame");
    await writeFile(join(dir, "assets", id + ".png"), "image");
    await writeFile(join(dir, "assets", other + ".png"), "keep");
    const beforeDelete = vi.fn();
    await expect(
      s.permanentlyDeleteArtwork({ id, confirmed: true }, beforeDelete),
    ).rejects.toThrow();
    await s.setArtworkDeleted({ id, deleted: true });
    await expect(
      s.permanentlyDeleteArtwork({ id, confirmed: false }, beforeDelete),
    ).rejects.toThrow();
    await expect(
      s.permanentlyDeleteArtwork(
        { id: "../outside", confirmed: true },
        beforeDelete,
      ),
    ).rejects.toThrow();
