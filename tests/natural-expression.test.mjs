...
test('cat and generic pet prompts preserve species and natural mouth instead of prescribing a dog smile',()=>{
 const r={name:'너부리',features:'고양이, 원본 눈 모양 유지',style:'pixel',mode:'generate'};
 const prompts=[buildPrompt(r),motionGroupPrompt({...r,action:'master'}),motionGroupPrompt({...r,action:'idle'}),motionPrompt(r.name)];
...
test('an explicit owner expression request is retained without changing other pets defaults',()=>{
 const r={name:'터치',features:'혀를 살짝 내밀고 웃는 표정',style:'pixel',mode:'generate'};
