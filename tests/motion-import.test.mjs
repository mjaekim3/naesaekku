...
async function sheet(columns,rows) {
 const marker=await sharp({create:{width:6,height:4,channels:4,background:'#fff'}}).png().toBuffer();
 const tile=await sharp({create:{width:24,height:32,channels:4,background:'#123456'}}).composite([{input:marker,left:3,top:4}]).png().toBuffer();
