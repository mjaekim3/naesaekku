...
import { PetModel, clampPosition, chooseDisplay, hitAlpha } from '../core/pet.mjs';
const displays = [
...
test('drag across negative-coordinate monitor retains chosen height and stays there', () => {
  const pet = new PetModel({ displays, x: 1500, y: 500, random: () => .5 });
  ...
  pet.act('walk'); pet.tick(1000);
  expect(pet.x).toBeGreaterThan(-650);
  expect(pet.y).toBe(200);
});
test('work area clamp and monitor removal recover lost pets', () => {
  expect(clampPosition({x:9999,y:9999},displays[1].workArea,220,230)).toEqual({x:-220,y:554});
...
test('sleep, feeding and petting complete without hunger penalties; drag has priority', () => {
  const p=new PetModel({displays,random:()=>.5});
  p.act('sleep');p.tick(1000);expect(p.state).toBe('sleep');
  p.act('eat');expect(p.state).toBe('eat');
  for(let i=0;i<80;i++)p.tick(100);
  expect(p.state).toBe('idle');
  p.act('pet');expect(p.state).toBe('happy');
  p.beginDrag({x:p.x+10,y:p.y+10});p.act('eat');expect(p.state).toBe('drag');
  p.endDrag();expect(p.state).toBe('idle');
  p.act('pause');for(let i=0;i<200;i++)p.tick(100);expect(p.state).toBe('idle');
  expect(p.roaming).toBe(false);
  p.act('walk');expect(p.roaming).toBe(true);
});
...
test('invalid saved positions fall back and snapshots only contain restorable data', () => {
  ...
  p.act('wake'); expect(p.state).toBe('idle');
});
